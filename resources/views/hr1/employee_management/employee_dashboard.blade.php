...
@section('content')
    <div class="p-10 container min-w-full bg-white rounded-lg shadow-md">
        ...
        @if (session('success'))
            <div class="mb-5 relative w-full overflow-hidden rounded-md border border-green-500 bg-white text-neutral-600 text-black"
                role="alert">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <div
                class="bg-blue-400 text-white p-6 rounded-xl shadow-md hover:bg-blue-300 transition duration-300 ease-in-out">
                <h2 class="text-lg font-semibold">Total Employees</h2>
                <p class="text-4xl mt-2">{{ $totalEmployees }}</p>
            </div>
            <div
                class="bg-green-500 text-white p-6 rounded-xl shadow-md hover:bg-green-400 transition duration-300 ease-in-out">
                <h2 class="text-lg font-semibold">Active Employees</h2>
                <p class="text-4xl mt-2">{{ $activeEmployees }}</p>
            </div>
            <div
                class="bg-red-500 text-white p-6 rounded-xl shadow-md hover:bg-red-400 transition duration-300 ease-in-out">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <a href="{{ route('employee.list') }}"
                class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-6 rounded-full transition transform hover:scale-105 shadow-lg text-center">
                View All Employees
            </a>
        ...
        <div x-data="{ selectedTab: 'departments' }" class="w-full">
            <div @keydown.right.prevent="$focus.wrap().next()" @keydown.left.prevent="$focus.wrap().previous()"
                class="flex gap-2 overflow-x-auto border-b border-neutral-300" role="tablist" aria-label="tab options">
                <button @click="selectedTab = 'departments'" :aria-selected="selectedTab === 'departments'"
                    :tabindex="selectedTab === 'departments' ? '0' : '-1'"
                    :class="selectedTab === 'departments' ? 'font-bold text-blue-500 border-b-2 border-blue-500' :
                    ...
                    aria-controls="tabpanelDepartments">
                    Departments
                    <span
                        :class="selectedTab === 'departments' ? 'text-white border-black bg-blue-500' :
                            'border-neutral-300 bg-neutral-100'"
                        class="text-xs font-medium px-1 rounded-full">{{ $totalDepartments }}</span>
                </button>
                <button @click="selectedTab = 'positions'" :aria-selected="selectedTab === 'positions'"
                    :tabindex="selectedTab === 'positions' ? '0' : '-1'"
                    :class="selectedTab === 'positions' ? 'font-bold text-blue-500 border-b-2 border-blue-500' :
                        'text-neutral-600 font-medium hover:border-b-2 hover:border-b-neutral-800 hover:text-neutral-900'"
                    class="flex h-min items-center gap-2 px-4 py-2 text-sm" type="button" role="tab"
                    aria-controls="tabpanelPositions">
                    Positions
                    <span
                        :class="selectedTab === 'positions' ? 'text-white border-black bg-blue-500' :
                            'border-neutral-300 bg-neutral-100'"
                        class="text-xs font-medium px-1 rounded-full">{{ $totalPositions }}</span>
                </button>
            </div>
            <div class="px-2 py-4 text-neutral-600">
                <div x-show="selectedTab === 'departments'" id="tabpanelDepartments" role="tabpanel"
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-4 flex items-center justify-center">
                                            <a href="{{ route('department.edit', $department->id) }}"
                                                class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-full transition transform hover:scale-105 shadow">
                                            ...
                                            <form action="{{ route('department.destroy', $department->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-full transition transform hover:scale-105 shadow">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                            ...
                            <tbody>
                                @foreach ($positions as $position)
                                    <tr class="hover:bg-blue-50 transition duration-300 ease-in-out">
                                        ...
                                        <td class="p-4 hidden md:table-cell text-gray-800">
                                            {{ number_format($position->base_salary, 2) }}</td>
                                        <td class="p-4 flex items-center justify-center">
