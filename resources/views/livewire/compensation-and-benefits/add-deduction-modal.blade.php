    ...
    @if ($isModalOpen)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-white rounded-lg w-96 p-6 shadow-lg">
                <h1 class="text-2xl font-bold mb-6">Add Deduction</h1>
                ...
                <form wire:submit.prevent="store" class="space-y-6">
                    ...
                    <!-- Deduction Name -->
                    <div>
                        <label for="deduction_name" class="block text-lg font-semibold mb-2">Deduction Name</label>
                        <input type="text" wire:model="deduction_name" id="deduction_name" class="outline-none border transition-all duration-200 ease-in-out border-gray-300 rounded-lg px-4 py-2 w-full focus:ring focus:ring-blue-300">
                        @error('deduction_name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div x-data="{ deduction_type: '', showFrequency: false }" class="space-y-4">
                        <!-- Deduction Type -->
                        <div>
                            <label for="deduction_type" class="block text-lg font-semibold mb-2">Deduction Type</label>
                            <select wire:model="deduction_type" x-model="deduction_type" @change="showFrequency = deduction_type === 'recurring'" id="deduction_type" class="border border-gray-300 rounded-lg px-4 py-2 w-full focus:ring focus:ring-blue-300">
                                <option value="one_time">One-Time</option>
                                <option value="recurring">Recurring</option>
                                <option value="recurring_indefinitely">Recurring Indefinitely</option>
                            </select>
                            @error('deduction_type') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
